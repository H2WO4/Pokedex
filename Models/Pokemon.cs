...
public class Pokemon : I_Battler
{
	...
	/// </summary>
	public Dictionary<Stat, int> StatBoosts { get; } =
		new()
		{
			{ Stat.Atk, 0 }, { Stat.Def, 0 },
			{ Stat.SpAtk, 0 }, { Stat.SpDef, 0 },
			{ Stat.Spd, 0 },
		};
	...
	/// </summary>
	/// <exception cref="ArgumentException">Throws if value is a flag of the Enum</exception>
	...
	{
		var total = 0;
		foreach ((Stat key, int value) in EVs)
		{
			if (key != stat)
				total += value;
		}
	...
	{
		return attacker.CalculateAffinity(Species.Types);
	}
